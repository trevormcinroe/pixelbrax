...
class Encoder(linen.Module):
  ...
  def __call__(self, x: jnp.ndarray):
    ...
    for features, stride in zip(self.features, self.strides):
      ...
      x = linen.Conv(features,
                  kernel_size=self.kernel_size,
                  strides=(stride, stride))(x)
      x = linen.relu(x)
    x = x.reshape((x.shape[0], -1))
    x = linen.Dense(self.z_dim)(x)
    x = linen.LayerNorm()(x)
    x = linen.tanh(x)
    ...
    x = MLP(
      ...
      kernel_init=jax.nn.initializers.lecun_uniform())(x)
...
def make_policy_network(
    param_size: int,
    obs_size: Union[int, Sequence[int]],
  ...
  if not isinstance(obs_size, int):
    print(f'Making CNN encoder for the policy function')
    policy_module = Encoder(features=[32, 32, 32, 32], strides=[2, 1, 1, 1], z_dim=50,
                             layer_sizes=list(hidden_layer_sizes) + [param_size], activation=activation,)
    ...
    dummy_obs = jnp.zeros(obs_size[1:])
...
def make_value_network(
  ...
  if not isinstance(obs_size, int):
    print(f'Making CNN encoder for the value function')
    value_module = Encoder(features=[32, 32, 32, 32], strides=[2, 1, 1, 1], z_dim=50,
                             layer_sizes=list(hidden_layer_sizes) + [1], activation=activation,)
    # value_module = MLP(
    ...
    def apply(processor_params, policy_params, obs):
      obs = preprocess_observations_fn(obs, processor_params)
      # obs = value_encoder.apply(encoder_params, obs)
