...
def compute_ppo_loss(
  ...
  # print(f'DATA SHAPE: {data.observation.shape}')
  data = jax.tree_util.tree_map(lambda x: jnp.swapaxes(x, 0, 1), data)
  # print(f'DATA SHAPE: {data.observation.shape} // {data.pixels.shape}')
  # print(f'DATA SHAPE: {data.observation.shape}')

  try:
    # The affine layers can be broadcast across multiple dims. convs cannot, so we need to reshape
    ps = data.pixels.shape
    policy_logits = policy_apply(normalizer_params, params.policy,
                                 data.pixels.reshape(ps[0] * ps[1], *ps[2:])).reshape(ps[0], ps[1], -1)
    baseline = value_apply(normalizer_params, params.value, data.pixels.reshape(ps[0] * ps[1], *ps[2:])).reshape(ps[0], ps[1])
  ...
  # print(f'pl: {policy_logits.shape}')
  # print(f'bl: {baseline.shape}')
  # print(f'bv: {bootstrap_value.shape}')
  # qqq
