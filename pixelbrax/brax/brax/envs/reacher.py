...
class Reacher(PipelineEnv):
  ...
  * v0: Initial versions release (1.0.0)
  ...
  def modified_step(self, state: State, action: jax.Array) -> State:
    pipeline_state = self.pipeline_step(state.pipeline_state, action)
    obs = self._get_obs(pipeline_state)
    ...
    # 0.08 is too easy..., 0.05 too hard?
    reward = (reward_dist >= -0.07).astype(jp.float32)
    # if reward_dist > -0.1:
    #   reward = 1.0
    # else:
    #   reward = 0.0
    # reward = 1 if reward_dist > -0.1 else 0
