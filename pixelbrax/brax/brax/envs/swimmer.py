...
class Swimmer(PipelineEnv):
  ...
  * A<sub>0</sub>, θ<sub>i</sub>: their derivatives with respect to time (velocity and angular velocity)
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    #
    forward_reward = self._forward_reward_weight * jp.linalg.norm(xy_position)
    # forward_reward = self._forward_reward_weight * pipeline_state0.obs[3] + pi
    obs = self._get_obs(pipeline_state)

    ctrl_cost = self._ctrl_cost_weight * jp.sum(jp.square(action))
    # forward_reward = self._forward_reward_weight * (jp.abs(obs[3]) + jp.abs(obs[4]))
